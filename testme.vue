		...
		async run() {
			...
			try {
				res1 = await axios.get(
					`${this.$APIServer}/club/${this.$store.state.ClubID}/shoot/${this.$store.state.ShootID}/scores/evnt/${this.Ev1.EvntId}?sort=name`
				);
				res2 = await axios.get(
					`${this.$APIServer}/club/${this.$store.state.ClubID}/shoot/${this.$store.state.ShootID}/scores/evnt/${this.Ev2.EvntId}?sort=name`
				);
			} catch (err) {
			...
			if (this.OnlyEligible) {
				...
				let dupscores = lodash.filter(
					lodash.uniq(
						lodash.map(data1, function (item) {
							if (
								lodash.filter(data1, {
				...
				let shootersWScore = lodash.filter(data1, (x) =>
					dupscores.includes(x.Score200)
				);

				let shooterWScoresIDs = lodash.map(shootersWScore, "ShooterId");
				console.log(shooterWScoresIDs);

				//GET THE SHOOTERS WITH THE SAME SCORES IN EVENT 2
				sameShootersInEvents = lodash.filter(data2, (x) =>
			...
			let blob = await ReportGen.getReport(
				`/Reports/Carryover${TrapNumber}Traps.docx`,
				reportData
			);

			//UPLOAD REPORT
			//UPLOAD FILE TO DOCGEN
			ReportGen.UploadFile(
				this.Ev2.EvntLabel,
